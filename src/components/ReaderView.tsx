...
interface ReaderViewProps {
  ...
  onUpdateProgress: (bookId: string, pageNumber: number, timeSpent?: number, totalPages?: number) => void;
...
export const ReaderView: React.FC<ReaderViewProps> = ({
  ...
  const recomputeReflow = useCallback(() => {
    ...
    setPages(newPages);
    if (newPages.length > 0 && newPages.length !== book.totalPages) {
      onUpdateProgressRef.current(book.id, lastSavedPageRef.current, sessionSecondsRef.current, newPages.length);
    }
  ...
  }, [book.id, currentPage, pages.length]);
  ...
  const isFinishedReading = pages.length > 1 && currentPage >= pages.length;
  const rawProgress = pages.length > 0 ? Math.round((currentPage / pages.length) * 100) : 0;
  const progressPercent = isFinishedReading ? 100 : Math.min(99, Math.max(1, rawProgress));
  ...
  // Estimated total reading duration
  const totalEstMins = Math.max(1, Math.round(totalBookWords / 220));
  const estHours = Math.floor(totalEstMins / 60);
  const estMinsRem = totalEstMins % 60;
  const estTotalBookDuration = estHours > 0 
    ? (estMinsRem > 0 ? `${estHours}h ${estMinsRem}m total` : `${estHours}h total`)
    : `${totalEstMins}m total`;
  ...
  return (
    ...
    >
      ...
      <main
        ...
        className="flex-1 w-full h-full flex flex-col justify-between items-center px-4 sm:px-8 py-8 sm:py-10 transition-colors duration-200 cursor-pointer overflow-hidden"
      >
        <div 
      ...
      >
        ...
        <div className="flex items-center justify-between text-[11px] font-mono text-[#888888] max-w-2xl mx-auto w-full px-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <span id="reading-stats">{estMinutesRemaining}m left • {estTotalBookDuration}</span>
          </div>
        </div>
      </footer>
